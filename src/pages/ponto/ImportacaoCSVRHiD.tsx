...
const TIPO_LABEL: Record<TipoDia, { label: string; cls: string }> = {
  normal: { label: "Normais", cls: "bg-success/10 text-success" }, falta: { label: "Faltas", cls: "bg-destructive/10 text-destructive" }, folga: { label: "Folgas", cls: "bg-muted text-muted-foreground" }, feriado: { label: "Feriados", cls: "bg-primary/10 text-primary" }, atestado: { label: "Atestados", cls: "bg-amber-500/10 text-amber-600" }, sem_vinculo: { label: "Sem vínculo", cls: "bg-muted text-muted-foreground" },
};
...
export default function ImportacaoCSVRHiD() {
  ...
  const handleSelect = async (selected: File | null) => {
    ...
    try {
      ...
      if (mensal.registros.length === 0) { toast({ title: "Arquivo incompatível", description: bruto.erros[0] || "Nenhum registro válido encontrado.", variant: "destructive" }); return; }
      setParse(mensal); setMaps(matchMaps); setPre(await preAnalisar(mensal, hash, matchMaps));
      toast({ title: "Relatório mensal compatível ✓", description: `${mensal.totalLinhas} linhas · ${mensal.cpfs.length} funcionários. Revise o resumo antes de confirmar.` });
  ...
  const handleConfirm = async () => {
    ...
    try {
      if (rawParse && rawPre) {
        ...
        toast({ title: result.importacaoId ? "Importação concluída" : "Falha na importação", description: result.importacaoId ? `${result.gravados} registros gravados no lote.` : result.erros[0] || "Erro desconhecido.", variant: result.importacaoId ? "default" : "destructive" });
      }
    } catch (error: any) { toast({ title: "Erro na importação", description: error?.message || "Não foi possível gravar o lote.", variant: "destructive" }); }
    finally { setSaving(false); }
  };

  return <AppLayout><div className="max-w-6xl mx-auto space-y-6">
    <div><h1 className="text-2xl font-bold tracking-tight">Importar marcações RHiD</h1><p className="text-sm text-muted-foreground">Envie o CSV bruto exportado do RHiD/ControlID. O sistema identifica o modelo, confere os vínculos e organiza as batidas para a apuração mensal.</p></div>
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6"><div className="lg:col-span-2 space-y-4">
      ...
      {rawStats && <div className="rounded-xl border bg-card p-6 shadow-sm space-y-4"><h3 className="font-bold text-lg">Importação automática concluída</h3><div className="grid grid-cols-2 sm:grid-cols-4 gap-3"><div className="p-3 rounded-lg bg-success/10"><p className="text-[10px] uppercase font-bold text-success">Batidas gravadas</p><p className="text-2xl font-bold text-success">{rawStats.gravados.toLocaleString("pt-BR")}</p></div><div className="p-3 rounded-lg bg-primary/10"><p className="text-[10px] uppercase font-bold text-primary">Vinculadas</p><p className="text-2xl font-bold text-primary">{rawStats.vinculadas.toLocaleString("pt-BR")}</p></div><div className="p-3 rounded-lg bg-warning/10"><p className="text-[10px] uppercase font-bold text-warning">Sem vínculo</p><p className="text-2xl font-bold text-warning">{rawStats.semVinculo}</p></div><div className="p-3 rounded-lg bg-muted/50"><p className="text-[10px] uppercase font-bold text-muted-foreground">Duplicadas</p><p className="text-2xl font-bold">{rawStats.duplicadas}</p></div></div>{rawStats.erros.length > 0 && <div className="text-[11px]"><p className="font-bold mb-1">Avisos ({rawStats.erros.length})</p><div className="max-h-32 overflow-auto rounded bg-muted p-2 font-mono whitespace-pre-wrap">{rawStats.erros.join("\n")}</div></div>}<div className="flex flex-wrap gap-2"><Link to="/ponto/apuracao"><Button variant="outline" className="gap-2">Abrir apuração mensal <ArrowRight className="h-3 w-3" /></Button></Link><Button variant="ghost" onClick={() => { setFile(null); reset(); }}>Importar outro arquivo</Button></div></div>}
      {stats && <div className="rounded-xl border bg-card p-6 shadow-sm space-y-4"><h3 className="font-bold text-lg">Resultado da importação</h3><div className="grid grid-cols-2 gap-4"><div className="p-3 rounded-lg bg-success/10"><p className="text-[10px] text-success uppercase font-bold">Registros gravados</p><p className="text-2xl font-bold text-success">{stats.gravados}</p></div><div className="p-3 rounded-lg bg-warning/10"><p className="text-[10px] text-warning uppercase font-bold">Sem vínculo</p><p className="text-2xl font-bold text-warning">{stats.funcionariosNaoEncontrados}</p></div></div>{stats.erros.length > 0 && <div className="text-[11px]"><p className="font-bold mb-1">Avisos ({stats.erros.length})</p><div className="max-h-40 overflow-auto rounded bg-muted p-2 font-mono whitespace-pre-wrap">{stats.erros.join("\n")}</div></div>}<div className="flex gap-2"><Link to="/ponto/relatorio-rhid"><Button variant="outline" className="gap-2">Consultar registros importados <ArrowRight className="h-3 w-3" /></Button></Link><Button variant="ghost" onClick={() => { setFile(null); reset(); }}>Importar outro arquivo</Button></div></div>}
    </div><div className="space-y-4"><div className="rounded-xl border bg-primary/5 p-5"><h4 className="font-bold text-sm mb-2 flex items-center gap-2"><FileSpreadsheet className="h-4 w-4" />Como funciona</h4><ul className="text-xs space-y-2.5 text-muted-foreground"><li>• O arquivo bruto é identificado pelas colunas <b>NSR, Data, PIS, CPF e Funcionário</b>.</li><li>• O vínculo usa CPF primeiro, depois PIS e nome somente quando não houver duplicidade.</li><li>• CPFs sem vínculo ficam preservados para revisão; nenhum funcionário é criado automaticamente.</li><li>• As batidas vinculadas recebem a obra atual do funcionário e entram na apuração mensal.</li><li>• A folha só deve receber os dados depois da conferência da apuração.</li></ul></div><div className="rounded-xl border p-4 text-xs text-muted-foreground flex gap-2"><Info className="h-4 w-4 flex-shrink-0 mt-0.5" /><span>O relatório mensal pré-calculado antigo continua compatível e segue disponível no mesmo importador.</span></div></div></div>
  </div></AppLayout>;
}
